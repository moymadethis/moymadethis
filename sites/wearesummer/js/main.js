...
/* 
 * Display a sliding line when navigation items are interacted with. This script
 * will slide the line along navigation items from an selected (current) menu
 * item. If not menu items are selected (so no line is present) it will fade the
 * line in on the initial item that's hovered over and then slide along to other
 * items when required. It will then fade out again if no link is clicked.
 *
 * 1. Only run above viewport widths of 1024px.
 *
 */
